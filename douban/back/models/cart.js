...
class Cart {
        ...
        pool.getConnection((err,connection)=>{
            ...
            connection.query(`select * from cart where uid=${uid} and pid=${pid}`,(err,results)=>{
                ...
                if(results.length){
                    //数量累加                
                    console.log(333)
                    var bbb = parseInt(number)
                    var newNumber = results[0].number+bbb
    ...
    // constructor(){}
    // add({pid,uid,shop},callback){
    //     pool.getConnection((err,connection)=>{
    //         if(err) throw err
    //         // 先看看  表里面  有没有添加过 当前的商品
    //         connection.query(`select * from cart where uid=${uid} and shop=${shop} and pid=${pid}`,(err,results)=>{
    //             if(err) throw err
    //             if(results.shop){
    //                 console.log(results)
    //                 if(results.length){
    ...
    //                     connection.query(`update cart set number=${newNumber} where uid=${uid} and shop=$(shop) and pid=${pid}`,(err)=>{
    //                         callback(err)
    //                         connection.release() //再回调里面 释放连接
    //                     })
    //                 }else{
    //                     connection.query(`insert into cart(uid,shop,pid,number) values(${uid},${shop},${pid},1)`,(err)=>{
    //                         callback(err)
    //                         connection.release() //再回调里面 释放连接
    //                     })
    //                 }
